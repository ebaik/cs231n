...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    reg = self.reg
    X = X.reshape(X.shape[0],-1)  # reshape X into a 2D array
    N, D = X.shape
    ...
    ############################################################################
    scores_stable = scores - np.amax(scores,axis=1,keepdims=True) # (N,C)
    ...
    dhidden_affine, dW2, db2 = affine_backward(dscores,cache_scores)
    dX, dW1, db1 = affine_relu_backward(dhidden_affine,cache_hidden)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # layer, etc.                                                              #
    ############################################################################
    Xgetshape = X.reshape(X.shape[0],-1)  # reshape X into a 2D array
    N, D = Xgetshape.shape
    reg = self.reg
    reg_loss = 0
    ...
    ############################################################################
    loss = data_loss + reg_loss
    dx,dw,db = affine_backward(dx,cache_affine_history[self.num_layers-1])
